...
export class Fire extends PIXI.Container {
  ...
  private _createEmitter(): void {
    ...
    this._emitter.scale.set(2.4);
    this._emitter.position.y = -170;
  ...
  private _updateGlow(dt: number): void {
    ...
    if (this._glowPauseTimer <= 0) {
      ...
      this._glowTargetAmp = Math.random() < 0.25 ? Utils.rand(0.0, 0.02) : Utils.rand(0.03, 0.12);
    }
    ...
    this._glow.alpha = gsap.utils.clamp(0.01, 0.3, this._glowBaseAlpha + wave * this._glowAmp);
